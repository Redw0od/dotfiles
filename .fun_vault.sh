...
# vault_curl <https://vault/path> [token]
vault_curl() {
  local cURL="${1}"
...
# Curl Vault with token
# vault_get </secret/path> [token]
vault_get() {
  local secret_path="${1}"
  local vToken="${2:-${VAULT_TOKEN}}"
  if [ -n "${secret_path}" ]; then
    vault_curl "${VAULT_ADDR}/v1/${secret_path#/*}" "${vToken}"
  fi
}

# Curl Vault with token
# vault_list </secret/path> [token]
vault_list () {
  local secret_path="${1}"
  local vToken="${2:-${VAULT_TOKEN}}"
  if [ -n "${secret_path}" ]; then
    vault_curl "${VAULT_ADDR}/v1/${secret_path#/*}" "${vToken}" "--request LIST"
  fi
}

# Copy vault secret from current vault cluster to another cluster
# vault_sync <target-vault_profile> </secret/path>
vault_sync() {
  ...
  local source_vault=${3:-$VAULT_PROFILE}
    ...
    if [[ "${list_json[@]}" ]]; then
      for secret_path in ${list_json[@]}; do
        ...
        vault_sync "${target_vault}" "${vault_path}" "${source_vault}"
      done
      return
    fi
    ...
    vault_set_endpoint "${full_secret_path%secret/*}secret/" "${endpoint_engine:-generic}" "${target_vault}"
...
vault_set_endpoint() {
  ...
  vault_profile "${original_profile}"
}

# Set Vault ENV variables per $VAULTS[?] index
# vault_profile <profile_name>
vault_profile() {
  local profile="${1^^}"
...
# vault_rds_lookup [customerId] [env]
vault_rds_lookup() {
...
vault_debug_logs() {
  local region=${1:-"us-west-2"}
  local name=${2:-"*vault_asg_node"}
  local nodes=$(vault_nodes ${region} ${name})
  for n in ${nodes[@]}; do
...
vault_check_binary() {
  if [ -n "$(shell_utilities 'vault' )" ]; then return 1; fi
